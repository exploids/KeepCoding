...
void setup() {
  ...
  filterImage = loadImage("filter.png");
  glowImage = loadImage("glow.png");
  errorIcon = loadImage("error_icon.png");
  titleFont = createFont("GlacialIndifference-Bold.otf", 36);
  dragSound = new SoundFile(this, "drag.mp3");
  dropSound = new SoundFile(this, "drop.mp3");
  solveSound = new SoundFile(this, "solve.mp3");
  collideSound = new SoundFile(this, "collide.mp3");
  things = new Thing[10];
...
void draw() {
  ...
  if (draggedThing != null) {
    if (isActive()) {
      draggedThing.move(mouseX + dragDeltaX, mouseY + dragDeltaY);
  ...
  if (time >= animationStart && time < animationEnd) {
    float boxWidth = (OUTER_MAX_X - OUTER_MIN_X) * factor;
    float boxHeight = (OUTER_MAX_Y - OUTER_MIN_Y) * factor;
    imageMode(CENTER);
...
void drawShapes() {
  background(REGULAR_BACKGROUND_COLOR);
...
void drawEnd() {
  background(solved ? SOLVED_BACKGROUND_COLOR : FAILED_BACKGROUND_COLOR);
  imageMode(CENTER);
  image(errorIcon, width * 0.5, INNER_MIN_Y + 100);
  imageMode(CORNER);
  fill(255);
  textFont(titleFont);
  ...
  text(text, width * 0.5, INNER_MIN_Y + 140);
}

void mousePressed() {
  if (isActive()) {
    for (int i = 0; i < things.length; i++) {
      if (things[i].collidesWith(mouseX, mouseY)) {
        draggedThing = things[i];
        dragDeltaX = draggedThing.x - mouseX;
        dragDeltaY = draggedThing.y - mouseY;
