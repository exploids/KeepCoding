...
void setup() {
  ...
  allGamenames.add("ArrangeShapes");
  ...
  for (int i = 1; i<=allGamenames.size(); i++) {
    ...
    if (allGamenames.get(i-1).equals("ArrangeShapes")) games.add( new ArrangeShapes((i%teileX)*gameSize+(i%teileX)*abstand*2+abstand, abstand+(i/teileX)*gameSize+(i/teileX)*abstand*2, this));
